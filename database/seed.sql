-- seed: popular pokemon cards with psa 9 -> psa 10 price upside
-- powers homepage "top regrade opportunities" grid
-- run: psql pokegrade -f database/seed.sql
-- idempotent via ON CONFLICT, safe to re-run

-- cert numbers are made up but in plausible psa format
-- prices are illustrative, not live market data
...
-- on re-run, refresh row to latest seed values
-- updated_at bumps so getSuggested ordering stays sensible
